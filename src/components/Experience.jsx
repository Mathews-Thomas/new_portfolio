    ...
    <div className="experience py-20 bg-gray-100" id="experience">
      <div className="container mx-auto px-6">
        ...
        <div className="timeline relative">
          {experiences.map((exp, index) => (
            <div key={index} className={`timeline-item w-full md:w-1/2 p-4 relative ${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8'}`}>
              <div className="timeline-text bg-white shadow-lg p-6 rounded-lg relative">
                <div className={`timeline-date text-red-600 font-semibold tracking-wide uppercase text-lg mb-4 absolute top-0 ${index % 2 === 0 ? 'right-0 transform translate-x-full' : 'left-0 transform -translate-x-full'}`}>
                  {exp.date}
                </div>
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 h-full bg-red-600 w-1"></div>
        </div>
      </div>
    </div>
  );
